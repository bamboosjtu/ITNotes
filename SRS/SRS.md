...
**以使用为中心**，通过场景来形成用例方法，以便理解用户需求，从而得到**功能需求**。
...
- ER图
- 数据字典：数据元素、描述、数据构成或数据类型、数据长度、数据取值。
- CRUD矩阵：分析系统事件、用户任务、用例等对数据实体的影响。
- 报表规范说明：
  - 报表ID
  - 报表标题
  - 报表用途
  ...
  - 直观布局
  - 页眉与页脚
  - 报表正体：过滤条件、包含字段、行列头部和布局、文本溢出的处理、排序标准、分组和小计、分页条件。
  - 报表结束标志
  - 互动性
  - 访问安全方面的限制
- 仪表板
...
用户非常憎恨使其生产力下降的**新的增强系统**，非功能性需求主要是质量属性。

Planguage语言提供一套关键词，可以精确描述质量属性和其他项目目标。



| 外部质量 | 简要描述                                                   |
| -------- | ---------------------------------------------------------- |
| 易用性   | 当在某时某地需要系统服务时，系统服务能够被有效访问的程度。 |
...
| 健壮性   | 系统如何应对非预期的操作。                                 |
| 安全性   | 如何防止系统被破坏性损坏。                                 |
| 防护性   | 系统如何组织对应用及其数据的未授权访问。                   |
| 易用性   | 用户对系统的学习、记忆和使用的难易程度。                   |
...
- 接口与集成
- 安全，如单点登录
- 通用的应用特性，如搜索、打印、文件操作、用户资料、撤销/重做、文本格式化
- 多平台



如果要重用需求，最好建立一个需求模式。模式定义了一个模板，其中包含项目中常见类型需求的各类信息，不同类型的需求模式有自己的内容类别。
...
需求管理实践包括：版本控制、变更控制（建立一套变更控制流程）、需求状态跟踪、需求追踪。



需求状态可采用：已提议、进行中、起草完成、已核准、已实现、已验证、已推迟、已删除、已驳回。

敏捷项目用户故事状态可采用：已定义、进行中、已完成、已验证、已阻塞。



需求追踪追踪的是需求与设计元素、代码、测试、文档之间的关系。
